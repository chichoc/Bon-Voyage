  ...
  try{
    ...
    $sql = "update phptest2.guide set subject=?, content=?, is_html=? where num=?";
    ...
    header("Location:http://localhost:8090/samjo/guide/list.php");
    ...
    try{
     $pdo->beginTransaction();   
     $sql = "insert into phptest2.guide(id,name,nick,subject,content,regist_day,hit,is_html) ";
     $sql .= "values(?, ?, ?, ?, ?, now(), 0, ?)";
     $stmh = $pdo->prepare($sql); 
