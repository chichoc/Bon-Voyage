<?php session_start(); ?>
 ...
 if(isset($_REQUEST["html_ok"]))  //checkbox는 체크해야 변수명 전달됨.
  ...
  $upload_dir = 'C:\xampp\htdocs\samjo\data\\';   //물리적 저장위치    
 ...
 if ($mode=="modify"){
    ...
    $del_ok[$index] = "y";
     ...
     try{
        $sql = "select * from phptest2.community where num=?";  // get target record
      ...
      unlink($delete_path);
         ...
         $pdo->beginTransaction(); 
                   $sql = "update phptest2.community set $field_org_name = ?, $field_real_name = ?  where num=?";
                   $stmh = $pdo->prepare($sql); 
     ...
     try{
        $pdo->beginTransaction();   
        $sql = "update phptest2.community set subject=?, content=?, is_html=? where num=?";
        $stmh = $pdo->prepare($sql); 
   ...
   try{
     $pdo->beginTransaction();
     $sql = "insert into phptest2.community(id, name, nick, subject, content, regist_day, hit, is_html, ";
   ...
   header("Location:http://localhost/samjo/community/list.php");
 ?>