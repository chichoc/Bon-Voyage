 ...
 if ($mode=="modify"){
   try{
     ...
     $sql = "update phptest2.qna set subject=?, content=?, is_html=? where num=?";
     ...
     header("Location:http://localhost/samjo/qna/list.php?page=$page");
   ...
   $is_html = "";
    ...
    {
     try{
       $sql = "select * from phptest2.qna where num = $num"; // 부모 글 가져오기
       $stmh = $pdo->prepare($sql);  
   ...
   // ord 가 부모글의 ord($row[ord]) 보다 큰 경우엔 ord 값 1 증가 시킴
       $pdo->beginTransaction();
       $sql = "update phptest2.qna set ord = ord + 1 where group_num = ? and ord > ?";
       $stmh = $pdo->prepare($sql); 
       ...
       $pdo->beginTransaction();  
       $sql = "insert into phptest2.qna(group_num, depth, ord, id, name, nick, subject,";
     ...
     try{
       ...
       $sql = "insert into phptest2.qna(depth,ord,id,name,nick,subject,content,regist_day,hit,is_html) ";
       $sql .= "values(?, ?, ?, ?, ?, ?, ?,now(), 0, ?)";
       $stmh = $pdo->prepare($sql); 
       ...
       $pdo->beginTransaction();   
       $sql = "update phptest2.qna set group_num = ? where num=?";
