<?php session_start(); ?>
 ...
 if ($mode=="modify"){
     ...
     try{
        $sql = "select * from phptest2.sales where num=?";  // get target record
      ...
      for ($i=0; $i<$count; $i++)						
      {
		...
		unlink($delete_path);
		   ...
		   $pdo->beginTransaction(); 
                   $sql = "update phptest2.sales set $field_org_name = ?, $field_real_name = ?  where num=?";
                   $stmh = $pdo->prepare($sql); 
     ...
     try{
        $pdo->beginTransaction();   
        $sql = "update phptest2.sales set subject=?, content=?, is_html=? where num=?";
        $stmh = $pdo->prepare($sql); 
   ...
   try{
     $pdo->beginTransaction();
     $sql = "insert into phptest2.sales(id, name, nick, subject, content, regist_day, hit, is_html, ";
   ...
   header("Location:http://localhost/samjo/sales/list.php");
 ?>
